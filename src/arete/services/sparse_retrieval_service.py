...
class BaseSparseRetriever(ABC):
    ...
    def build_index(self, chunks: List[Chunk]) -> None:
        ...
        try:
            logger.info(
                f"Building {self.get_algorithm_name()} index from {len(chunks)} chunks"
            )
            ...
            raise IndexingError(
                f"Failed to build {self.get_algorithm_name()} index: {e}"
            ) from e
    ...
    def search(
        ...
        try:
            ...
            for chunk_id, document in self.index.documents.items():
                ...
                if (
                    chunk_types
                    and document.metadata.get("chunk_type") not in chunk_types
                ):
                    continue
            ...
            avg_relevance = sum(score for _, score in scored_results) / max(
...
class SPLADERetriever(BaseSparseRetriever):
    ...
    def score_document(
        ...
        normalized_score = min(
            1.0, max(0.0, total_score / max(len(query_terms) * 10, 1))
        )

        return normalized_score

    def _calculate_term_importance(
        self, term: str, document: TermFrequencyDocument
    ) -> float:
        """
...
class SparseRetrievalService:
    ...
    def __init__(
        ...
        **retriever_kwargs,
        ...
        logger.info(
            f"Initialized SparseRetrievalService with {retriever_type} retriever"
        )
