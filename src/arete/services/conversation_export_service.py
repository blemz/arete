...
class ConversationExportService:
    ...
    def export_session(
        ...
        try:
            ...
            file_path = os.path.join(
                options.output_directory or self.temp_dir, filename
            )
    ...
    def _generate_filename(
        self, session: ChatSession, format: ExportFormat, options: ExportOptions
    ) -> str:
    ...
    def _export_to_markdown(
        ...
        if options.include_context and session.context:
            ...
            if session.context.philosophical_period:
                lines.append(
                    f"- **Philosophical Period**: {session.context.philosophical_period}"
                )
            ...
            if session.context.learning_objectives:
                lines.append(
                    f"- **Learning Objectives**: {', '.join(session.context.learning_objectives)}"
                )
            lines.append("")
        ...
        if not session.messages:
            ...
            for message in session.messages:
                ...
                if (
                    ...
                    and not options.include_system_messages
                ):
                    continue
                ...
                if options.format_philosophical_quotes:
                    ...
                    if message.message_type == MessageType.ASSISTANT and any(
                        word in content.lower()
                        for word in ["aristotle", "plato", "kant", "socrates"]
                    ):
                        content = f"> {content}"
    ...
    def _export_to_html(
        ...
        if options.include_metadata:
            ...
            html_parts.append(
                f"                <li><strong>Session ID:</strong> {session.session_id}</li>"
            )
            html_parts.append(
                f"                <li><strong>Created:</strong> {session.created_at.strftime('%Y-%m-%d %H:%M:%S')}</li>"
            )
            html_parts.append(
                f"                <li><strong>Messages:</strong> {len(session.messages)}</li>"
            )
            html_parts.append("            </ul>")
        ...
        if not session.messages:
            ...
            for message in session.messages:
                ...
                html_parts.append(
                    f'                <div class="message-header">{message.message_type.value.title()}{timestamp_str}</div>'
                )
                html_parts.append(
                ...
                if options.include_citations and message.citations:
                    ...
                    for citation in message.citations:
                        citation_class = (
                            "highlighted" if options.highlight_citations else ""
                        )
                        html_parts.append(
                            f'                        <li class="{citation_class}">{citation}</li>'
                        )
                    html_parts.append("                    </ul>")
                    html_parts.append("                </div>")
    ...
    def _export_to_pdf(
        ...
        for message in session.messages:
            ...
            for word in words:
                if (
                    pdf.get_string_width(line + word + " ") < 180
