...
class ProviderConfigurationService:
    ...
    def create_configuration(
        ...
        """
        config_data = {"provider": provider, "api_key": api_key, **kwargs}
    ...
    def update_configuration(self, provider: str, **updates) -> ProviderConfiguration:
        """
    ...
    def set_active_provider(
        ...
        logger.info(
            f"Set active provider: {provider}" + (f", model: {model}" if model else "")
        )
    ...
    async def check_provider_health(
        ...
        health = self._health_status.get(
            provider,
            ProviderHealth(
                provider=provider, status=ProviderStatus.UNKNOWN, last_check=now
            ),
        )
        ...
        if self.config_file.exists():
            try:
                ...
                for provider, config_data in data.items():
                    self._configurations[provider] = ProviderConfiguration(
                        **config_data
                    )
        ...
        if self.health_file.exists():
            try:
                ...
                for provider, health_data in data.items():
                    self._health_status[provider] = ProviderHealth.from_dict(
                        health_data
                    )
                logger.info(
                    f"Loaded health status for {len(self._health_status)} providers"
                )
            except Exception as e:
    ...
    def _save_health_status(self) -> None:
        ...
        try:
            ...
            logger.debug(
                f"Saved health status for {len(self._health_status)} providers"
            )
        except Exception as e:
