...
class SimpleLLMService:
    ...
    async def generate_response(
        ...
        **kwargs,
        ...
        try:
            ...
            logger.info(
                f"Generating response with {provider_name}"
            ...
            response.metadata.update(
                {
                    ...
                    "model_selected_by": (
                        "user"
                        if model
                        else ("default" if model_name else "provider_default")
                    ),
            ...
            logger.info(
                f"Generated response: {len(response.content)} chars, {response.usage_tokens} tokens"
            )
            return response
    ...
    def configure_provider(
        ...
        if existing_config:
            ...
            self.config_service.update_configuration(
                provider, api_key=api_key, **config_options
            )
        else:
            # Create new configuration
            self.config_service.create_configuration(
                provider, api_key=api_key, **config_options
            )
    ...
    async def validate_provider_configuration(self, provider: str) -> Dict[str, Any]:
        ...
        health_status = await self.config_service.check_provider_health(
            provider, force=True
        )
...
async def quick_generate(prompt: str, provider: Optional[str] = None, **kwargs) -> str:
    ...
    response = await service.generate_response(
        messages=messages, provider=provider, **kwargs
    )
...
def show_provider_status() -> None:
    ...
    print(
        f"  Environment Variable: {info['provider_source']['env_variable'] or 'Not set'}"
    )
    print(
        f"  Settings Selected: {info['provider_source']['settings_selected'] or 'Not set'}"
    )
    print(f"  Settings Default: {info['provider_source']['settings_default']}")
    print()
    print("Model Configuration Source:")
    print(
        f"  Environment Variable: {info['model_source']['env_variable'] or 'Not set'}"
    )
    print(
        f"  Settings Selected: {info['model_source']['settings_selected'] or 'Not set'}"
    )
    print()
...
def set_provider_interactive() -> None:
    ...
    try:
        choice = input(
            f"\nSelect provider (1-{len(info['available_providers'])}): "
        ).strip()
        index = int(choice) - 1
