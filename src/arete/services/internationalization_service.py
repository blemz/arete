...
class InternationalizationService:
    ...
    def _initialize_language_info(self):
        ...
        languages = [
            ...
            LanguageInfo(
                ...
                DateFormat.ISO,
                decimal_separator=".",
                philosophical_tradition="Chinese Philosophy",
            ),
            LanguageInfo(
                "zh-TW",
                "Chinese (Traditional)",
            ...
            LanguageInfo(
                ...
                DateFormat.EUROPEAN,
                decimal_separator=".",
                philosophical_tradition="Islamic Philosophy",
            ),
            LanguageInfo(
                "he",
                "Hebrew",
    ...
    def _load_translations(self):
        ...
        for key, text in base_translations.items():
            self.translations["en"][key] = TranslationEntry(
                key=key, text=text, context="interface"
            )
    ...
    def translate(
        ...
        **kwargs,
        ...
        if (
            target_language in self.translations
            and key in self.translations[target_language]
        ):
            translation = self.translations[target_language][key].text
        # Fallback to default language
        elif (
            self.default_language.value in self.translations
            and key in self.translations[self.default_language.value]
        ):
                ...
                for key, value in data.items():
                    if isinstance(value, str):
                        self.translations[language_code][key] = TranslationEntry(
                            key=key, text=value, context="loaded"
                        )
