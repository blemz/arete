    ...
    async def generate_response(
        ...
        **kwargs,
        ...
        if preferred_provider:
            preferred = next(
                (p for p in self._providers if p.name == preferred_provider), None
            )
        ...
        for provider in providers_to_try:
            ...
            try:
                ...
                logger.info(
                    f"Successfully generated response with provider: {provider.name}"
                )
                return response
        ...
        for provider in self._providers:
            try:
                ...
                provider_statuses.append(
                    {"provider": provider.name, "status": "error", "error": str(e)}
                )

        healthy_count = sum(
            1 for status in provider_statuses if status.get("status") == "healthy"
        )
...
def create_llm_service(settings: Settings | None = None) -> MultiProviderLLMService:
    ...
    try:
        default_provider = service.factory.create_provider(
