...
class ClusterInfo:
    ...
    def calculate_similarity(self, other: "ClusterInfo") -> float:
        ...
        similarity = cosine_similarity(
            [self.center_embedding], [other.center_embedding]
        )[0][0]
...
class DiversityResult:
    ...
    def get_final_score(
        self, strategy: str = "balanced", relevance_weight: float = 0.7
    ) -> float:
        """
        ...
        if strategy == "relevance_only":
            ...
            return (
                self.original_result.relevance_score * relevance_weight
...
class DiversityMetrics:
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            / self.total_diversification_requests,
            "total_results_processed": self.total_results_processed,
            "average_diversity_score": self.average_diversity_improvement
            / self.total_diversification_requests,
...
class DiversityService:
    ...
    def __init__(self, config: DiversityConfig, settings: Settings):
        """
    ...
    def diversify(
        ...
        try:
            ...
            if method == DiversityMethod.MMR:
                ...
                diversified_results = self._semantic_distance_diversification(
                    search_results
                )
            ...
            self._update_metrics(
                method.value, len(search_results), processing_time, diversified_results
            )

            logger.info(
                f"Diversified {len(search_results)} results to {len(diversified_results)} using {method.value}"
            )
            ...
            if results:
                return [
                    self._create_diversity_result(
                        results[0], 0, 1.0, 0.0, 1.0, 1.0, 1.0
                    )
                ]
            return []
        ...
        while remaining and len(selected) < self.config.max_results:
            ...
            for candidate in remaining:
                ...
                mmr_score = (
                    ...
                    + (1 - self.config.lambda_param) * diversity
                )
            ...
            if best_result and best_diversity > (
    ...
    def _clustering_diversification(
        ...
        for cluster_id, cluster_info in clusters.items():
            ...
            if len(cluster_results) >= self.config.min_cluster_size:
                ...
                diversity_score = 0.9 - (
                    distance * 0.3
                )  # Convert distance to diversity score
                diversity_results.append(
                    self._create_diversity_result(
                        best_result,
                        cluster_id,
        ...
        diversity_results.sort(
            key=lambda x: x.original_result.relevance_score, reverse=True
        )
    ...
    def _semantic_distance_diversification(
        ...
        while remaining_indices and len(selected) < self.config.max_results:
            ...
            for idx in remaining_indices:
                ...
                for selected_result in selected:
                    selected_embedding = selected_result.chunk.embedding_vector
                    distance = 1.0 - self._calculate_cosine_similarity(
                        candidate_embedding, selected_embedding
                    )
                ...
                combined_score = (
                    candidate.relevance_score * 0.5 + semantic_novelty * 0.5
                )
            ...
            if best_idx is not None and best_novelty > (
                1.0 - self.config.similarity_threshold
            ):
    ...
    def _hybrid_diversification(
        ...
        for i, result in enumerate(combined_results):
            ...
            semantic_novelty = self._calculate_semantic_novelty(
                result.original_result, [r.original_result for r in combined_results]
            )
    ...
    def _calculate_overall_diversity_score(
        ...
        for selected_result in selected_results:
            if selected_result != target_result:
                selected_embedding = selected_result.chunk.embedding_vector
                similarity = self._calculate_cosine_similarity(
                    target_embedding, selected_embedding
                )
    ...
    def _calculate_semantic_novelty(
        ...
        if (
            not other_results
            or target_result in other_results
            and len(other_results) == 1
        ):
            return 1.0
    ...
    def _update_metrics(
        ...
        if diversified_results:
            avg_diversity = sum(r.diversity_score for r in diversified_results) / len(
                diversified_results
            )
