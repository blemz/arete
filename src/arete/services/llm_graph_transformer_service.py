...
class PhilosophicalLLMGraphTransformer:
    ...
    def _create_transformer(self) -> LLMGraphTransformer | None:
        ...
        try:
            ...
            print(
                f"[OK] Successfully initialized LLMGraphTransformer with {kg_provider} ({kg_model})"
            )
            return transformer
    ...
    def _create_langchain_llm(self):
        ...
        if provider == "openai":
            ...
            return ChatAnthropic(
                model=model, temperature=0.0, api_key=self.config.anthropic_api_key
            )
            ...
            print(
                f"[WARN] Provider {provider} not supported for LLMGraphTransformer, using OpenAI fallback"
            )
            return ChatOpenAI(
                model="gpt-4o-mini", temperature=0.0, api_key=self.config.openai_api_key
            )
    ...
    async def extract_graph_from_text(
        ...
        print(
            f"[DEBUG] Transformer has convert_to_graph_documents: {hasattr(self.transformer, 'convert_to_graph_documents')}"
        )
        ...
        print(
            f"[INFO] Processing first {max_chunks} chunks (rate limiting for testing)"
        )
        ...
        for i, chunk in enumerate(chunks[:max_chunks]):
            print(
                f"  [INFO] Processing chunk {i+1}/{max_chunks} (size: {len(chunk)} chars)..."
            )
            ...
            try:
                ...
                try:
                    ...
                    kg_model = (
                        self.config.kg_llm_model or self.config.selected_llm_model
                    )
                    ...
                    if (
                        ...
                        print(
                            f"    [ERROR] LLM transform failed for chunk {i+1}: {transform_error}"
                        )
                ...
                if graph_docs is None or not isinstance(graph_docs, list):
                    print(
                        f"    [WARN] No valid graph documents returned for chunk {i+1} (got type: {type(graph_docs)})"
                    )
                    continue
                ...
                entities, relationships = self._process_graph_documents(
                    graph_docs, document_id
                )
                ...
                print(
                    f"    [OK] Found {len(entities)} entities, {len(relationships)} relationships"
                )
        ...
        print(
            f"[RESULT] Final results: {len(merged_entities)} unique entities, {len(validated_relationships)} validated relationships"
        )
...
Analysis:"""
        ...
        try:
            ...
            response = await self.llm_service.generate_response(
                messages=messages, max_tokens=2000, temperature=0.1, timeout=120
            )
        ...
        try:
            ...
            for ent_data in data.get("entities", []):
                entity = Entity(
                    ...
                    mentions=[],
                    confidence=0.8,
                    description=ent_data.get(
                        "description", ""
                    ),  # Use description field directly
                )
                entities.append(entity)
...
class LLMGraphTransformerService:
    ...
    async def extract_knowledge_graph(
        ...
        Returns:
            Tuple of (entities, relationships)
        """
        return await self.transformer.extract_graph_from_text(
