...
class QualityMonitor:
    ...
    async def _run_evaluation_cycle(self) -> None:
        ...
        try:
            ...
            self.logger.info(
                f"Completed evaluation cycle with {len(evaluation_results)} results"
            )
    ...
    async def _check_quality_alerts(self, results: List[EvaluationResult]) -> None:
        ...
        current_metrics = {
            ...
            "answer_relevancy": statistics.mean(
                [r.answer_relevancy_score for r in results]
            ),
            "context_precision": statistics.mean(
                [r.context_precision_score for r in results]
            ),
            "context_recall": statistics.mean(
                [r.context_recall_score for r in results]
            ),
            "overall_quality": statistics.mean(
                [r.overall_quality_score for r in results]
            ),
        }
        ...
        alert_checks = [
            ...
            (
                "answer_relevancy",
                current_metrics["answer_relevancy"],
                thresholds.answer_relevancy_min,
                AlertSeverity.WARNING,
            ),
            (
                "context_precision",
                current_metrics["context_precision"],
                thresholds.context_precision_min,
                AlertSeverity.WARNING,
            ),
            (
                "context_recall",
                current_metrics["context_recall"],
                thresholds.context_recall_min,
                AlertSeverity.WARNING,
            ),
            (
                "overall_quality",
                current_metrics["overall_quality"],
                thresholds.overall_quality_min,
                AlertSeverity.CRITICAL,
            ),
        ]
    ...
    async def _check_quality_drops(self, current_metrics: Dict[str, float]) -> None:
        ...
        historical_metrics = {
            "faithfulness": statistics.mean(
                [r.faithfulness_score for r in recent_history]
            ),
            "answer_relevancy": statistics.mean(
                [r.answer_relevancy_score for r in recent_history]
            ),
            "overall_quality": statistics.mean(
                [r.overall_quality_score for r in recent_history]
            ),
        }
        ...
        return {
            ...
            "statistics": {
                ...
                "last_evaluation_time": (
                    stats.last_evaluation_time.isoformat()
                    if stats.last_evaluation_time
                    else None
                ),
            },
            "recent_alerts": [
    ...
    def get_monitoring_stats(self) -> MonitoringStats:
        ...
        if self.evaluation_history:
            ...
            last_evaluation_time = max(
                [r.evaluation_timestamp for r in self.evaluation_history]
            )
    ...
    async def run_manual_evaluation(
        ...
        try:
            ...
            self.logger.info(
                f"Manual evaluation completed with {len(evaluation_results)} results"
            )
