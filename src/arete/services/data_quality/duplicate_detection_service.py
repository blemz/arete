...
class SimilarityMetrics(BaseModel):
    ...
    jaccard_similarity: float = Field(
        ..., ge=0.0, le=1.0, description="Jaccard similarity coefficient"
    )
    cosine_similarity: float = Field(
        ..., ge=0.0, le=1.0, description="Cosine similarity score"
    )
    levenshtein_similarity: float = Field(
        ..., ge=0.0, le=1.0, description="Normalized Levenshtein similarity"
    )
    ...
    calculation_method: str = Field(
        default="combined", description="Method used for calculation"
    )
    confidence_score: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Confidence in similarity assessment"
    )
...
class DuplicateResult(AreteBaseModel):
    ...
    confidence_level: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Confidence in duplicate detection"
    )
    false_positive_risk: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Estimated risk of false positive"
    )
...
class DeduplicationResult(BaseModel):
    ...
    final_count: int = Field(
        ..., description="Final number of items after deduplication"
    )
    removal_rate: float = Field(
        ..., ge=0.0, le=1.0, description="Percentage of items removed"
    )

    # Strategy used
    keep_strategy: KeepStrategy = Field(
        ..., description="Strategy used for item selection"
    )

    # Processing metadata
    processing_time_ms: float = Field(..., description="Time taken for deduplication")
    duplicate_groups_processed: int = Field(
...
class DuplicateDetectionService:
    ...
    def _initialize_ml_models(self):
        ...
        if SENTENCE_TRANSFORMERS_AVAILABLE:
            try:
                ...
                self.logger.warning(
                    f"Failed to initialize sentence transformer: {str(e)}"
                )
                self.sentence_model = None
        else:
            self.sentence_model = None
            self.logger.warning(
                "Sentence transformers not available - semantic similarity will be limited"
            )
    ...
    def find_exact_duplicates(
        ...
        self.logger.info(
            f"Found {len(duplicate_results)} exact duplicate groups from {len(items)} items"
        )
            ...
            for i in range(len(items)):
                ...
                if similar_indices:
                    ...
                    avg_similarity = float(
                        np.mean([similarity_matrix[i][j] for j in similar_indices])
                    )
    ...
    def find_fuzzy_duplicates(
        ...
        for i in range(len(items)):
            ...
            if len(similar_items) > 1:
                avg_similarity = (
                    fuzzy_threshold  # Simplified - could calculate actual average
                )
    ...
    async def _calculate_semantic_similarity(
        self, item1: Any, item2: Any, field: str = "text"
    ) -> float:
    ...
    def _deduplicate_results(
        ...
        for result in results:
            ...
            item_ids = sorted(
                [str(getattr(item, "id", hash(str(item)))) for item in result.items]
            )
    ...
    async def deduplicate_items(
        ...
        # Find duplicates
        duplicates = await self.detect_all_duplicates(
            items, detection_strategies, field
        )
        ...
        for duplicate_group in duplicates:
            if len(duplicate_group.items) > 1:
                # Apply keep strategy
                item_to_keep = self._apply_keep_strategy(
                    duplicate_group.items, keep_strategy
                )
                items_to_remove = [
                    item for item in duplicate_group.items if item != item_to_keep
                ]
    ...
    def calculate_custom_similarity(
        self, text1: str, text2: str, function_name: str
    ) -> float:
    ...
    def _monitor_performance(
        ...
        self.logger.info(
            f"Performance - {operation_name}: {processing_time:.2f}s for {items_count} items ({throughput:.1f} items/s)"
        )
