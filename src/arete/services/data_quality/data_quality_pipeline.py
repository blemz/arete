...
class QualityAssessmentReport(BaseModel):
    ...
    ragas_status: ValidationStatus = Field(
        ValidationStatus.SKIPPED, description="RAGAS validation status"
    )
    ...
    duplicate_status: ValidationStatus = Field(
        ValidationStatus.SKIPPED, description="Duplicate detection status"
    )
    ...
    citation_status: ValidationStatus = Field(
        ValidationStatus.SKIPPED, description="Citation validation status"
    )
    ...
    def calculate_overall_score(self) -> float:
        ...
        if self.deduplication_summary:
            duplicate_rate = (
                len(self.deduplication_summary.removed_items)
                / (
                    len(self.deduplication_summary.kept_items)
                    + len(self.deduplication_summary.removed_items)
                )
                if (
                    self.deduplication_summary.kept_items
                    or self.deduplication_summary.removed_items
                )
                else 0.0
            )
...
class QualityValidationRules(BaseModel):
    ...
    min_faithfulness: float = Field(
        0.7, description="Minimum faithfulness score", ge=0.0, le=1.0
    )
    min_answer_relevancy: float = Field(
        0.7, description="Minimum answer relevancy", ge=0.0, le=1.0
    )
    min_context_precision: float = Field(
        0.6, description="Minimum context precision", ge=0.0, le=1.0
    )
    min_context_recall: float = Field(
        0.6, description="Minimum context recall", ge=0.0, le=1.0
    )

    # Philosophical quality thresholds
    min_argument_coherence: float = Field(
        0.6, description="Minimum argument coherence", ge=0.0, le=1.0
    )
    min_conceptual_clarity: float = Field(
        0.7, description="Minimum conceptual clarity", ge=0.0, le=1.0
    )
    min_textual_fidelity: float = Field(
        0.8, description="Minimum textual fidelity", ge=0.0, le=1.0
    )

    # Duplicate detection thresholds
    max_duplicate_rate: float = Field(
        0.1, description="Maximum allowed duplicate rate", ge=0.0, le=1.0
    )
    semantic_similarity_threshold: float = Field(
        0.85, description="Semantic similarity threshold", ge=0.0, le=1.0
    )
    fuzzy_matching_threshold: float = Field(
        0.8, description="Fuzzy matching threshold", ge=0.0, le=1.0
    )

    # Citation validation thresholds
    min_citation_accuracy: float = Field(
        0.8, description="Minimum citation accuracy", ge=0.0, le=1.0
    )
    max_citation_errors: int = Field(
        5, description="Maximum citation errors allowed", ge=0
    )

    # Performance thresholds
    max_processing_time: float = Field(
        300.0, description="Maximum processing time in seconds"
    )
    min_throughput_items_per_second: float = Field(
        0.1, description="Minimum processing throughput"
    )
    ...
    def validate_ragas_results(
        ...
        if results.faithfulness_score < self.min_faithfulness:
            issues.append(
                f"Faithfulness {results.faithfulness_score:.3f} below threshold {self.min_faithfulness}"
            )
            status = ValidationStatus.FAILED

        if results.answer_relevancy_score < self.min_answer_relevancy:
            issues.append(
                f"Answer relevancy {results.answer_relevancy_score:.3f} below threshold {self.min_answer_relevancy}"
            )
            status = ValidationStatus.FAILED

        if results.context_precision_score < self.min_context_precision:
            issues.append(
                f"Context precision {results.context_precision_score:.3f} below threshold {self.min_context_precision}"
            )
            status = (
        ...
        if results.context_recall_score < self.min_context_recall:
            issues.append(
                f"Context recall {results.context_recall_score:.3f} below threshold {self.min_context_recall}"
            )
            status = (
                ValidationStatus.WARNING
                if status != ValidationStatus.FAILED
    ...
    def validate_duplicate_results(
        ...
        duplicate_items = sum(
            len(dup.items) - 1 for dup in duplicates
        )  # -1 because we keep one from each group
        duplicate_rate = duplicate_items / total_items

        if duplicate_rate > self.max_duplicate_rate:
            issues.append(
                f"Duplicate rate {duplicate_rate:.3f} exceeds threshold {self.max_duplicate_rate}"
            )
    ...
    def validate_citation_accuracy(
        ...
        if accuracy < self.min_citation_accuracy:
            issues.append(
                f"Citation accuracy {accuracy:.3f} below threshold {self.min_citation_accuracy}"
            )
            status = ValidationStatus.FAILED

        if error_count > self.max_citation_errors:
            issues.append(
                f"Citation errors {error_count} exceed threshold {self.max_citation_errors}"
            )
            status = (
                ValidationStatus.WARNING
                if status != ValidationStatus.FAILED
...
class QualityPipelineConfig(BaseModel):
    ...
    enable_duplicate_detection: bool = Field(
        True, description="Enable duplicate detection"
    )
    enable_citation_validation: bool = Field(
        True, description="Enable citation validation"
    )
    enable_quality_monitoring: bool = Field(
        True, description="Enable quality monitoring"
    )
...
class DataQualityPipeline:
    ...
    async def assess_data_quality(
        ...
        report = QualityAssessmentReport(
            assessment_id=assessment_id, assessment_level=self.config.assessment_level
        )
    ...
    def _categorize_data(
        ...
        categorized = {"documents": [], "chunks": [], "citations": [], "other": []}
    ...
    async def _run_assessment_components(
        ...
        if self.config.enable_quality_monitoring and self.config.assessment_level in [
            QualityAssessmentLevel.COMPREHENSIVE,
            QualityAssessmentLevel.RESEARCH,
        ]:
    ...
    async def _run_ragas_evaluation(
        ...
        try:
            ...
            ragas_result = await self.ragas_service.evaluate_query(
                question=query, contexts=contexts or [], ground_truth=ground_truth
            )

            report.ragas_results = ragas_result

            # Validate against thresholds
            status, issues = self.config.validation_rules.validate_ragas_results(
                ragas_result
            )
    ...
    async def _run_duplicate_detection(
        ...
        try:
            ...
            for data_type, items in data_by_type.items():
                ...
                for strategy in self.config.duplication_strategies:
                    if strategy == DuplicationStrategy.EXACT_MATCH:
                        duplicates = self.duplicate_service.find_exact_duplicates(
                            items, field=text_field
                        )
                    elif strategy == DuplicationStrategy.SEMANTIC_SIMILARITY:
                        duplicates = (
                            await self.duplicate_service.find_semantic_duplicates(
                                items, field=text_field
                            )
                        )
            ...
            self.logger.info(
                f"Duplicate detection completed: {len(all_duplicates)} groups found"
            )
    ...
    async def _run_citation_validation(
        ...
        try:
            self.logger.info(
                f"Running citation validation for {len(citations)} citations"
            )
            ...
            for citation in citations:
                ...
                if not self._is_valid_classical_reference(citation.target_passage):
                    citation_errors.append(
                        f"Invalid reference format: {citation.target_passage}"
                    )
                    continue
                ...
                if hasattr(citation, "confidence") and citation.confidence < 0.7:
                    citation_errors.append(
                        f"Low confidence citation: {citation.target_passage}"
                    )
                    continue
    ...
    def _generate_recommendations(self, report: QualityAssessmentReport):
        ...
        if (
            ...
            and report.processing_time
            > self.config.validation_rules.max_processing_time
        ):
            report.add_recommendation(
    ...
    async def run_continuous_assessment(
        self, data_source: callable, assessment_interval: int = 3600  # 1 hour
    ):
        """Run continuous data quality assessment."""
        self.logger.info(
            f"Starting continuous assessment with {assessment_interval}s interval"
        )
