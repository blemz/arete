...
class ResponseValidation:
    ...
    def is_high_quality(
        self, accuracy_threshold: float = 0.8, citation_threshold: float = 0.7
    ) -> bool:
        """Check if response meets high quality standards."""
        return (
            self.is_valid
            and self.accuracy_score >= accuracy_threshold
            and self.citation_coverage >= citation_threshold
...
class ResponseResult:
    ...
    validation: ResponseValidation = field(
        default_factory=lambda: ResponseValidation(
...
class ResponseGenerationService:
    ...
    def __init__(
        ...
        self.citation_extraction_service = (
            citation_extraction_service or CitationExtractionService()
        )
        self.citation_validation_service = (
            citation_validation_service or CitationValidationService()
        )
        self.citation_tracking_service = (
            citation_tracking_service or CitationTrackingService()
        )
        ...
        try:
            ...
            if generation_config.enable_caching:
                cache_key = self._generate_cache_key(
                    context_result, query, generation_config
                )
                cached_result = self._get_cached_result(cache_key)
                if cached_result:
                    logger.debug(
                        f"Returning cached response for query: {query[:50]}..."
                    )
            ...
            extraction_result = (
                self.citation_extraction_service.extract_citations_from_response(
            ...
            if extraction_result.citations:
                validation_results = (
                    await self.citation_validation_service.validate_citations_batch(
            ...
            context_citations = self._process_citations(
                context_result, generation_config
            )
            ...
            if generation_config.enable_validation and self.validation_service:
                try:
                    ...
                    if (
                        ...
                        and generation_config.fail_on_validation_error
                    ):
        ...
        try:
            ...
            for i, result in enumerate(results):
                if isinstance(result, Exception):
                    logger.error(
                        f"Batch generation failed for query '{queries[i]}': {result}"
                    )
                    # Create error result
        ...
        for i, citation in enumerate(citations, 1):
            if format_style == "classical":
                ...
                formatted_citations.append(
                    citation.source_reference or str(citation.id)
                )
    ...
    async def _validate_response(
        ...
        if not self.validation_service:
            ...
            return ResponseValidation(
                ...
                citation_coverage=len(response_result.citations)
                / max(len(context_result.citations), 1),
            )
    ...
    def get_service_stats(self) -> Dict[str, Any]:
        ...
        return {
            "llm_service_info": (
                self.llm_service.get_provider_info() if self.llm_service else {}
            ),
