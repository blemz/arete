...
class ConversationSharingService:
    ...
    def access_share(
        ...
        if not share.can_access(user_email):
            return (
                None,
                "Access denied - you are not authorized to view this conversation",
            )
    ...
    def update_share_permissions(
        self, share_id: str, owner_user_id: str, new_permissions: SharePermissions
    ) -> bool:
    ...
    def extend_share_expiration(
        self, share_id: str, owner_user_id: str, new_expiration: ExpirationPeriod
    ) -> bool:
    ...
    def add_authorized_user(
        self, share_id: str, owner_user_id: str, user_email: str
    ) -> bool:
    ...
    def get_share_analytics(self, share_id: str, owner_user_id: str) -> Dict[str, any]:
        ...
        return {
            ...
            "days_until_expiration": (
                (share.expires_at - datetime.now()).days if share.expires_at else None
            ),
        }
    ...
    def generate_sharing_url(
        self, share_id: str, base_url: str = "https://arete.app/shared"
    ) -> str:
    ...
    def get_share_statistics(self) -> Dict[str, any]:
        ...
        total_accesses = sum(
            share.access_count for share in self._shared_conversations.values()
        )
        ...
        return {
            ...
            "total_comments": sum(
                len(comments) for comments in self._share_comments.values()
            ),
            "total_annotations": sum(
                len(annotations) for annotations in self._share_annotations.values()
            ),
        }
