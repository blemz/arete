...
class CitationExtractionService:
    ...
    def __init__(self, config: CitationExtractionConfig | None = None):
        ...
        self._classical_patterns = [
            re.compile(pattern, re.IGNORECASE) for pattern in self.CLASSICAL_PATTERNS
        ]
        self._quote_patterns = [re.compile(pattern) for pattern in self.QUOTE_PATTERNS]
        self._author_work_patterns = [
            re.compile(pattern, re.IGNORECASE) for pattern in self.AUTHOR_WORK_PATTERNS
        ]

        logger.info(
            f"Initialized CitationExtractionService with {len(self._classical_patterns)} classical patterns"
        )

    def extract_citations_from_response(
        self, response_text: str, context_result: ContextResult, query: str = ""
    ) -> ExtractionResult:
        """
        ...
        try:
            ...
            result.accuracy_score = self._calculate_accuracy_score(
                validated_citations, context_result
            )
            result.coverage_score = self._calculate_coverage_score(
                validated_citations, context_result
            )
    ...
    def _find_citation_matches(self, text: str) -> list[CitationMatch]:
        ...
        if self.config.enable_direct_quote_detection:
            for pattern in self._quote_patterns:
                for match in pattern.finditer(text):
                    ...
                    if (
                        self.config.min_citation_length
                        <= len(quote_text)
        ...
        matches = [
            m for m in matches if m.confidence >= self.config.min_confidence_threshold
        ]
            ...
            if work and work.lower() in citation.source_title.lower():
                ...
                if reference:
                    if (
                        ...
                        and reference in citation.source_reference
                    ):
                        return citation
                else:
    ...
    def _calculate_accuracy_score(
        ...
        high_confidence_citations = sum(1 for c in citations if c.confidence >= 0.8)
    ...
    def _analyze_quality_issues(
        ...
        if self.config.enable_classical_patterns and not any(
            c.citation_type == CitationType.REFERENCE for c in result.citations
        ):
        ...
        quote_count = sum(
            1 for c in result.citations if c.citation_type == CitationType.DIRECT_QUOTE
        )
