...
class HistoricalDevelopmentService:
            ...
            if match:
                try:
                    if "BCE" in date_string.upper() or "BC" in date_string.upper():
                        ...
                        if (
                            year < 500
                            and "CE" not in date_string.upper()
                            and "AD" not in date_string.upper()
                        ):
        ...
        try:
            async with self.client.session() as session:
                ...
                if start_year or end_year:
                    events = self._filter_events_by_date_range(
                        events, start_year, end_year
                    )
                ...
                concept_evolutions = await self._analyze_concept_evolutions(
                    session, events
                )
    ...
    async def _extract_historical_events(
        ...
        for record in records:
            ...
            try:
                entity_type = (
                    EntityType(entity_type_str) if entity_type_str else EntityType.OTHER
                )
            except ValueError:
            ...
            # Parse date
            date, confidence = (
                self._parse_date(date_string) if date_string else (None, 0.0)
            )
            ...
            for i in range(len(concept_event_list) - 1):
                ...
                if current.date and next_event.date:
                    influence_chain.append(
                        (current.entity_id, next_event.entity_id, current.date)
                    )
    ...
    async def trace_concept_development(
        self, concept_id: str, include_influences: bool = True
    ) -> ConceptEvolution:
        """
                    ...
                    if record["related_id"] and record["date_string"]:
                        ...
                        if date:
                            event = HistoricalEvent(
                                ...
                                or record["related_id"],
                                entity_type=(
                                    EntityType(record["related_type"])
