...
class KnowledgeGraphService:
    ...
    def __init__(
        ...
        self.entity_extractor = entity_extractor or EntityExtractor(
            use_philosophical_patterns=True
        )
    ...
    async def _extract_from_text(
        ...
        if enable_batching:
            ...
            entity_name_to_id = (
                await self.entity_repository.find_or_create_entities_by_name(
                    entity_names, EntityType.CONCEPT, document_id
                )
            )
            result.entities_found = len(
                [name for name in entity_names if name in entity_name_to_id]
            )
            ...
            for entity in entities:
                try:
                    existing_entities = await self.entity_repository.get_by_name(
                        entity.name
                    )
                    ...
                    result.add_warning(
                        f"Failed to process entity {entity.name}: {str(e)}"
                    )

        # Step 3: Extract relationships
        entity_names = list(entity_name_to_id.keys())
        raw_triples = self.relationship_extractor.extract_relationships(
            text, entity_names
        )
        ...
        if validated_triples:
            if enable_batching:
                result.relationships_created = (
                    await self.entity_repository.batch_create_triples(
        ...
        logger.info(
            f"Knowledge graph extraction completed: {result.entities_created} entities created, "
    ...
    async def _extract_from_chunks(
        ...
        chunker = ChunkingStrategy.get_chunker(
            "sliding_window", chunk_size=chunk_size, overlap=chunk_size // 4
        )
        ...
        validated_triples = self.triple_validator.validate(
            global_triples, min_confidence
        )
        result.triples_validated = len(validated_triples)
        ...
        result.entities_found = len(
            [name for name in global_entity_names if name in entity_name_to_id]
        )
        result.entities_created = len(entity_name_to_id) - result.entities_found
        ...
        logger.info(
            f"Chunked knowledge graph extraction completed: {result.entities_created} entities created, "
    ...
    async def get_entity_relationships(
        ...
        try:
            ...
            logger.error(
                f"Failed to get entity relationships for {entity_id}: {str(e)}"
            )
            raise
    ...
    async def analyze_philosophical_network(
        ...
        try:
            ...
            entities = await self.entity_repository.list_all(
                limit=1000, filters=filters
            )
            ...
            for entity in entities:
                ...
                relationships = await self.entity_repository.get_relationships(
                    entity.id
                )
                if len(relationships) >= min_relationships:
                    filtered_entities.append(
                        {"entity": entity, "relationship_count": len(relationships)}
                    )
            ...
            total_relationships = sum(
                e["relationship_count"] for e in filtered_entities
            )
            avg_relationships = (
