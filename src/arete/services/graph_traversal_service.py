...
class EntityDetector:
    ...
    def __init__(self, settings: Optional[Settings] = None):
        ...
        self.work_patterns = [r"\b(Republic|Ethics|Critique|Meditations|Apology)\b"]
    ...
    def detect_entities(self, text: str) -> List[EntityMention]:
        ...
        # Detect persons
        entities.extend(
            self._detect_by_patterns(text, self.person_patterns, EntityType.PERSON, 0.8)
        )
        ...
        # Detect works
        entities.extend(
            self._detect_by_patterns(text, self.work_patterns, EntityType.WORK, 0.9)
        )

        # Detect places
        entities.extend(
            self._detect_by_patterns(text, self.place_patterns, EntityType.PLACE, 0.8)
        )
...
class CypherQueryGenerator:
    ...
    def generate_entity_lookup(self, entities: List[EntityMention]) -> CypherQuery:
        ...
        return CypherQuery(
            cypher=cypher.strip(), parameters=parameters, query_type="entity_lookup"
        )
    ...
    def generate_relationship_traversal(
        ...
        if len(entities) < 2:
            ...
            return self._generate_single_entity_relations(
...
class GraphTraversalService:
    ...
    def is_initialized(self) -> bool:
        ...
        return (
            ...
            and self.entity_detector is not None
    ...
    def detect_entities(self, query_text: str) -> List[EntityMention]:
        ...
        try:
            ...
            logger.debug(
                f"Detected {len(entities)} entities in query: {query_text[:50]}..."
            )
            return entities
    ...
    def generate_cypher_query(
        ...
        try:
            ...
            if query.estimated_complexity > self.max_query_complexity:
                logger.warning(
                    f"Query complexity {query.estimated_complexity} exceeds limit {self.max_query_complexity}"
                )
                # Fall back to simpler query
                query = self.query_generator.generate_entity_lookup(entities)

            logger.debug(
                f"Generated {query_type} query with complexity {query.estimated_complexity}"
            )
            return query
    ...
    def integrate_with_search_results(
        ...
        try:
            ...
            integrated_results.sort(
                key=lambda r: getattr(r, "enhanced_score", r.final_score), reverse=True
            )
    ...
    def _calculate_graph_enhanced_score(
        ...
        enhanced_score = min(
            base_score + relationship_boost + confidence_boost - path_penalty, 1.0
        )
            ...
            try:
                ...
                entity = Entity(
                    ...
                    source_document_id=UUID(
                        entity_data.get("source_document_id", str(uuid4()))
                    ),
                )
...
def create_graph_traversal_service(
    ...
    return GraphTraversalService(neo4j_client=neo4j_client, settings=settings)
