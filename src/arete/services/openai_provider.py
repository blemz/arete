...
class OpenAIProvider(LLMProvider):
    ...
    def supported_models(self) -> List[str]:
        ...
        if (
            self._models_cache
            and (time.time() - self._models_cache_time) < self._cache_ttl
        ):
    ...
    def build_request_params(
        ...
        **kwargs,
        ...
        if max_tokens is not None:
            ...
            if model and model in legacy_models:
                ...
                is_reasoning_model = model and any(
                    reasoning_model in model for reasoning_model in reasoning_models
                )
                if is_reasoning_model:
                    ...
                    payload["max_completion_tokens"] = (
                        20000  # Balance between completeness and performance
                    )
                else:
        ...
        if temperature is not None:
            ...
            has_fixed_temp = model and any(
                fixed_temp_model in model for fixed_temp_model in models_with_fixed_temp
            )
    ...
    async def _handle_http_error(self, error: httpx.HTTPStatusError):
        ...
        if status_code == 401:
            raise AuthenticationError(
                f"OpenAI authentication failed: {error_message}", "openai"
            )
            ...
            raise RateLimitError(
                f"OpenAI rate limit exceeded: {error_message}", retry_seconds
            )
        elif status_code == 503:
            raise ProviderUnavailableError(
                f"OpenAI API unavailable: {error_message}", "openai"
            )
        else:
