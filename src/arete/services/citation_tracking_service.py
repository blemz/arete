...
class CitationTrackingService:
    ...
    def record_citation_event(
        ...
        try:
            ...
            if (
                len(self._provenance_records[citation.id])
                > self.config.max_provenance_records
            ):
                self._provenance_records[citation.id] = self._provenance_records[
                    citation.id
                ][-self.config.max_provenance_records :]
            ...
            logger.debug(
                f"Recorded {event_type.value} event for citation {citation.id}"
            )
            return record
    ...
    def create_citation_relationship(
        ...
        try:
            ...
            for citation_id in [source_citation.id, target_citation.id]:
                ...
                existing = any(
                    rel.source_citation_id == relationship.source_citation_id
                    and rel.target_citation_id == relationship.target_citation_id
        ...
        try:
            ...
            for citation_id in citation_ids:
                ...
                for rel in relationships:
                    ...
                    if (
                        rel.source_citation_id in citation_ids
                        and rel.target_citation_id in citation_ids
                    ):
            ...
            for rel in all_relationships:
                key = (
                    rel.source_citation_id,
                    rel.target_citation_id,
            ...
            if (
                include_automatic_relationships
                and self.config.enable_automatic_relationship_detection
            ):
    ...
    def get_citation_usage_stats(self, citation_id: UUID) -> CitationUsageStats:
        ...
        return self._usage_stats.get(
            citation_id, CitationUsageStats(citation_id=citation_id)
        )
    ...
    def analyze_citation_impact(
        ...
        try:
            ...
            impact_score = self._calculate_impact_score(
                citation_id, stats, relationships
            )
            influence_score = self._calculate_influence_score(
                citation_id, relationships
            )
    ...
    def _update_usage_stats(
        self, citation: Citation, event_type: TrackingEventType
    ) -> None:
        ...
        stats.average_confidence = (
            stats.average_confidence * (total_events - 1) + citation.confidence
        ) / total_events
    ...
    def _calculate_citation_similarity(
        ...
        source_sim = 0.0
        if (
            citation1.source_author == citation2.source_author
            and citation1.source_title == citation2.source_title
        ):
            source_sim = 1.0
    ...
    def _calculate_network_metrics(self, network: CitationNetwork) -> CitationNetwork:
        ...
        for citation in network.citations:
            degree = sum(
                ...
                if rel.source_citation_id == citation.id
                or rel.target_citation_id == citation.id
            )
    ...
    def _calculate_recent_usage(self, citation_id: UUID, cutoff_date: datetime) -> int:
        ...
        return sum(
            ...
            and record.event_type == TrackingEventType.REFERENCED
        )
