...
class PromptVersioningService:
    ...
    def __init__(self, config: VersioningConfig):
        ...
        logger.info(
            f"PromptVersioningService initialized with storage at {self.storage_path}"
        )
    ...
    def create_version(self, version: PromptVersion) -> bool:
        ...
        try:
            ...
            if version.version_id in existing_ids:
                logger.warning(
                    f"Version {version.version_id} already exists for {template_name}"
                )
                return False
    ...
    def rollback_to_version(
        self, template_name: str, target_version_id: str
    ) -> RollbackResult:
        """
        ...
        try:
            ...
            if self.create_version(rollback_version):
                logger.info(
                    f"Rollback successful: {current_version.version_id} -> {target_version_id}"
                )
                return RollbackResult(
                    success=True,
                ...
                return RollbackResult(
                    success=False, error_message="Failed to create rollback version"
                )

        except Exception as e:
            logger.error(f"Rollback failed: {e}")
            return RollbackResult(success=False, error_message=str(e))
    ...
    def compare_versions(
        ...
        recommendation = self._generate_comparison_recommendation(
            version1, version2, differences, performance_comparison
        )
    ...
    def delete_version(self, template_name: str, version_id: str) -> bool:
        ...
        self.versions[template_name] = [
            v for v in self.versions[template_name] if v.version_id != version_id
        ]
    ...
    def save_to_storage(self) -> bool:
        ...
        try:
            ...
            backup_file = (
                self.storage_path
                / f"prompt_versions_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            )
            ...
            data = {
                "metadata": {
                    ...
                    "version_count": sum(
                        len(versions) for versions in self.versions.values()
                    ),
            ...
            logger.info(
                f"Saved {data['metadata']['version_count']} versions to storage"
            )
            return True
    ...
    def cleanup_old_versions(self, days_to_keep: int = None) -> int:
        ...
        for template_name in list(self.versions.keys()):
            ...
            for version in self.versions[template_name]:
                ...
                if (
                    version == self.versions[template_name][0]
                    or version.metadata.is_rollback
                    or version.metadata.created_at > cutoff_date
                ):
                    versions_to_keep.append(version)
                else:
                    cleaned_count += 1
                    logger.info(
                        f"Cleaning up old version {version.version_id} from {template_name}"
                    )
    ...
    def _sort_versions(self, template_name: str) -> None:
        ...
        if template_name in self.versions:
            self.versions[template_name].sort(
                key=lambda v: v.get_semantic_version(), reverse=True
            )
    ...
    def _enforce_version_limit(self, template_name: str) -> None:
        ...
        if (
            ...
            and len(self.versions[template_name])
    ...
    def _generate_comparison_recommendation(
        ...
        for metric, values in performance_comparison.items():
            ...
            if v2_score > v1_score * 1.1:  # 10% improvement threshold
                recommendations.append(
                    f"{version2.version_id} shows {metric} improvement ({v2_score:.2f} vs {v1_score:.2f})"
                )
            elif v1_score > v2_score * 1.1:
                recommendations.append(
                    f"{version1.version_id} shows better {metric} ({v1_score:.2f} vs {v2_score:.2f})"
                )
        ...
        if "system_prompt" in differences:
            ...
            if length_change > 50:
                recommendations.append(
                    f"{version2.version_id} has significantly expanded prompt (+{length_change} chars)"
                )
            elif length_change < -50:
                recommendations.append(
                    f"{version2.version_id} has more concise prompt ({length_change} chars)"
                )
...
def create_versioning_service(
    storage_path: str, **config_kwargs
