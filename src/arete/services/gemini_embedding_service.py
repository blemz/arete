...
class GeminiEmbeddingService:
    ...
    def __init__(
        ...
        if (
            ...
            and self.api_model_name not in self.EMBEDDING_MODELS
        ):
            logger.warning(
                f"Unknown Gemini model {model_name}, using default dimensions"
            )
            self.dimensions = 768  # Default
        else:
            self.dimensions = self.EMBEDDING_MODELS.get(
                model_name, self.EMBEDDING_MODELS.get(self.api_model_name, 768)
            )
