...
class EnhancedKnowledgeGraphService:
    ...
    def __init__(self, llm_service: SimpleLLMService | None = None):
        ...
        if self.config.kg_llm_provider and self.config.kg_llm_model:
            print(
                f"INFO: Using dedicated KG LLM: {self.config.kg_llm_provider}/{self.config.kg_llm_model}"
            )
            # Create specialized LLM service for knowledge extraction
            self.llm_service = self._create_kg_llm_service()
        else:
            print(
                f"INFO: Using general LLM for KG extraction: {self.config.selected_llm_provider}/{self.config.selected_llm_model}"
            )
    ...
    def _initialize_transformer(self):
        ...
        try:
            ...
            if self.llm_transformer.is_available():
                print(
                    "✅ Successfully initialized LLMGraphTransformerService with philosophical schema"
                )
            else:
                print(
                    "⚠️  LLMGraphTransformerService initialized but LangChain transformer not available - will use fallback"
                )
    ...
    def _create_kg_llm_service(self) -> SimpleLLMService:
        ...
        print(
            f"DEBUG: Configured KG service - Provider: {kg_llm_service.get_active_provider_name()}, Model: {kg_llm_service.get_active_model_name()}"
        )
    ...
    async def extract_knowledge_graph(
        ...
        if max_chunks is not None and len(chunks) > max_chunks:
            print(
                f"INFO: Limiting processing to first {max_chunks} chunks out of {len(chunks)} total chunks"
            )
        ...
        for i, chunk in enumerate(chunks):
            ...
            try:
                ...
                print(
                    f"    Found {len(entities)} entities, {len(relationships)} relationships"
                )
    ...
    async def _extract_from_chunk(
        ...
        try:
            ...
            entities, relationships = (
                await self.llm_transformer.extract_knowledge_graph(
                    text=text,
            ...
            print(
                f"    ✅ LLMGraphTransformer extracted {len(entities)} entities, {len(relationships)} relationships"
            )
        ...
        for line in lines:
            ...
            if current_section == "entities" and "|" in line:
                ...
                if len(parts) >= 3:
                    ...
                    if self._is_valid_philosophical_entity(
                        subject
                    ) and self._is_valid_philosophical_entity(obj):
                        relationship = {
                            "subject": subject,
                            "relation": relation,
                            "object": obj,
    ...
    def _validate_relationships(
        ...
        for rel in relationships:
            ...
            if (
                self._is_valid_philosophical_entity(rel["subject"])
                and self._is_valid_philosophical_entity(rel["object"])
                and rel["confidence"] >= 0.5
            ):
