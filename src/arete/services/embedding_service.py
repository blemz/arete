...
class EmbeddingService:
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized EmbeddingService with model={self.model_name}, device={self.device}"
        )
    ...
    def load_model(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"Model loaded successfully. Dimension: {self.get_embedding_dimension()}"
            )
    ...
    def get_model_info(self) -> dict[str, Any]:
        ...
        return {
            ...
            "embedding_dimension": (
                self.get_embedding_dimension() if self.is_model_loaded() else None
            ),
    ...
    def generate_embeddings_batch(
        ...
        try:
            ...
            if show_progress:
                logger.info(
                    f"Generated {len(all_embeddings)} embeddings in {self._batch_count} batches"
                )
...
def get_embedding_service(
    ...
    if _embedding_service_instance is None:
        _embedding_service_instance = EmbeddingService(
            model_name=model_name, device=device, settings=settings
        )
