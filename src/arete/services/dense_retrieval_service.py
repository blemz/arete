...
class RetrievalMetrics:
    ...
    def update_query_metrics(
        self, results_count: int, avg_relevance: float, response_time: float
    ) -> None:
        ...
        if self.total_results_returned > 0:
            self.average_relevance_score = (
                self._relevance_sum / self.total_results_returned
            )
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_results_per_query": self.total_results_returned
            / max(self.queries_processed, 1),
...
class DenseRetrievalService:
    ...
    def __init__(
        ...
        if embedding_repository is None:
            self.embedding_repository = create_embedding_repository(
    ...
    def search_by_text(
        ...
        try:
            ...
            for chunk, relevance in raw_results:
                result = SearchResult(
                    chunk=chunk, relevance_score=relevance, query=query
                )
            ...
            if enhance_scores or custom_scorer:
                search_results = self._enhance_result_scores(
                    search_results, processed_query, custom_scorer
                )

            # Rank and filter results
            search_results = self._rank_and_filter_results(
                search_results, limit, min_relevance
            )

            # Expand context if requested
            if expand_context and search_results:
                search_results = self._expand_context_window(
                    search_results, context_window
                )

            # Update metrics
            response_time = time.time() - start_time
            avg_relevance = sum(r.final_score for r in search_results) / max(
    ...
    def search_by_vector(
        ...
        **kwargs,
        ...
        try:
            ...
            for chunk, relevance in raw_results:
                result = SearchResult(
                    chunk=chunk, relevance_score=relevance, query="[vector query]"
                )
                search_results.append(result)
        ...
        for result in results:
            ...
            if hasattr(self, "_enhance_relevance_score"):
                enhanced_score = self._enhance_relevance_score(
                    result.chunk, enhanced_score, query
                )
    ...
    def _enhance_relevance_score(
        self, chunk: Chunk, base_score: float, query: str
    ) -> float:
        """
    ...
    def _rank_and_filter_results(
        ...
        try:
            ...
            filtered_results = [
                result for result in results if result.final_score >= min_relevance
            ]
...
def create_dense_retrieval_service(
    ...
    return DenseRetrievalService(
        embedding_repository=embedding_repository, settings=settings
    )
