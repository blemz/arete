...
class ExpertAnnotation(BaseModel):
    ...
    timestamp: datetime = Field(
        default_factory=datetime.utcnow, description="When annotation was created"
    )
...
class ValidationItem(BaseModel):
    ...
    id: UUID = Field(
        default_factory=uuid4, description="Unique identifier for validation item"
    )
    item_type: str = Field(
    ...
    extraction_confidence: float = Field(
        ..., ge=0.0, le=1.0, description="Original extraction confidence"
    )
    ...
    def calculate_consensus(self) -> float:
        ...
        approved_count = sum(
            1 for ann in self.annotations if ann.status == ValidationStatus.APPROVED
        )
...
class ExpertValidationService:
    ...
    def __init__(self):
        ...
        self.requires_expert_threshold = (
            0.5  # Low confidence items require expert review
        )
    ...
    def submit_for_validation(
        ...
        logger.info(
            f"Submitted {item_type} for validation: {validation_item.id} "
    ...
    def submit_expert_annotation(
        ...
        logger.info(
            f"Expert annotation added by {expert_name} for item {validation_id}: {status}"
        )
    ...
    def _evaluate_consensus(self, validation_item: ValidationItem) -> None:
        ...
        if consensus_score >= self.min_consensus_threshold:
            ...
            logger.info(
                f"Item {validation_item.id} approved with consensus {consensus_score:.2f}"
            )
        else:
            # Check if majority rejects
            rejected_count = sum(
                1
                for ann in validation_item.annotations
            ...
            if rejected_count / len(validation_item.annotations) >= 0.5:
                ...
                logger.info(
                    f"Item {validation_item.id} rejected with consensus {consensus_score:.2f}"
                )
            else:
    ...
    def get_pending_validations(
        ...
        for validation_item in self._validation_queue.values():
            # Filter by status
            if validation_item.status not in [
    ...
    def assign_expert(self, expert_id: str, validation_ids: List[UUID]) -> bool:
        ...
        for validation_id in validation_ids:
            if validation_id not in self._validation_queue:
                logger.error(
                    f"Cannot assign non-existent validation item: {validation_id}"
                )
                return False
        ...
        logger.info(
            f"Assigned {len(validation_ids)} validation items to expert {expert_id}"
        )
    ...
    def get_validation_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "pending_count": sum(
                ...
                for item in self._validation_queue.values()
                if item.status
                in [ValidationStatus.PENDING, ValidationStatus.EXPERT_REVIEW]
            ),
        }
    ...
    def get_approved_items(self) -> List[ValidationItem]:
        ...
        return [
            item
            for item in self._validation_queue.values()
            if item.final_status == ValidationStatus.APPROVED
    ...
    def export_validation_data(self) -> Dict[str, Any]:
        ...
        return {
            "validation_items": [
                item.dict() for item in self._validation_queue.values()
            ],
