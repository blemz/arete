...
class OpenRouterEmbeddingService:
    ...
    def __init__(
        ...
        if model_name not in self.EMBEDDING_MODELS:
            logger.warning(
                f"Unknown OpenRouter model {model_name}, using default dimensions"
            )
    ...
    async def generate_embedding(self, text: str) -> List[float]:
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            try:
                ...
                logger.debug(
                    f"Generated OpenRouter embedding with {len(embedding)} dimensions"
                )
                return embedding
            ...
            for i in range(0, len(texts), self.max_batch_size):
                ...
                if not non_empty_batch:
                    ...
                    try:
                        ...
                        logger.debug(
                            f"Generated {len(non_empty_batch)} OpenRouter embeddings in batch"
                        )

                    except httpx.HTTPStatusError as e:
                        logger.error(
                            f"OpenRouter batch HTTP error: {e.response.status_code} - {e.response.text}"
                        )
                        # Return zero vectors for failed batch
                        batch_embeddings = [[0.0] * self.dimensions] * len(batch)
                    except Exception as e:
                        logger.error(
                            f"OpenRouter batch embedding generation failed: {e}"
                        )
                        # Return zero vectors for failed batch
                        batch_embeddings = [[0.0] * self.dimensions] * len(batch)
