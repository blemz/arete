...
class AnthropicEmbeddingService:
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized Anthropic embedding service with model: {model_name} ({self.dimensions}d)"
        )
        logger.warning(
    ...
    def _text_to_embedding(self, text: str) -> List[float]:
        ...
        word_features = [
            ...
            / max(len(words), 1),  # Average word length
    ...
    async def generate_embedding(self, text: str) -> List[float]:
        ...
        logger.debug(
            f"Generated Anthropic-style embedding with {len(embedding)} dimensions"
        )
        return embedding
