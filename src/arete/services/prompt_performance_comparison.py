...
class PromptPerformanceComparison:
    ...
    async def run_provider_comparison(
        ...
        if cache_results and cache_key in self.comparison_cache:
            logger.info(
                f"Returning cached comparison result for {len(providers)} providers"
            )
            return self.comparison_cache[cache_key]

        logger.info(
            f"Running performance comparison: {len(providers)} providers, {len(test_cases)} test cases"
        )
        ...
        for provider in providers:
            ...
            for i in range(0, len(test_cases), batch_size):
                ...
                batch_metrics = await self._run_provider_batch(
                    provider, batch, include_metrics
                )
                all_metrics.extend(batch_metrics)

            # Calculate provider comparison
            if all_metrics:
                provider_comparison = self._calculate_provider_comparison(
                    provider, all_metrics
                )
    ...
    async def _evaluate_single_test(
        ...
        token_count = response.llm_response.metadata.get(
        ...
        if MetricType.ALL in include_metrics or MetricType.ACCURACY in include_metrics:
            accuracy_score = await self._evaluate_accuracy(
                response.content, test_case.expected_concepts
            )

        if (
            MetricType.ALL in include_metrics
            or MetricType.CITATION_QUALITY in include_metrics
        ):
            citation_quality = self._evaluate_citation_quality(
                response.citations_provided
            )

        if (
            MetricType.ALL in include_metrics
            or MetricType.EDUCATIONAL_VALUE in include_metrics
        ):
            educational_value = self._evaluate_educational_value(
                response.content, test_case.student_level
            )
    ...
    async def _evaluate_accuracy(
        ...
        for concept in expected_concepts:
            ...
            if (
                concept_lower in response_lower
                or any(keyword in response_lower for keyword in concept_lower.split())
                or self._check_concept_variations(concept_lower, response_lower)
            ):
    ...
    def _evaluate_educational_value(self, response: str, student_level: str) -> float:
        ...
        educational_value = (
            length_score * 0.3 + indicator_score * 0.4 + structure_score * 0.3
        )
    ...
    def _analyze_semantic_consistency(self, sentences: list[str]) -> float:
        ...
        consistent_sentences = sum(
            1 for topic_dict in sentence_topics if primary_topic in topic_dict
        )
    ...
    def _analyze_argument_structure(
        ...
        has_conclusion = any(
            indicator in response_lower for indicator in conclusion_indicators
        )
        has_premise = any(
            indicator in response_lower for indicator in premise_indicators
        )
        ...
        philosophical_moves_count = sum(
            1 for move in philosophical_moves if move in response_lower
        )
    ...
    def _calculate_provider_comparison(
        ...
        avg_educational_value = statistics.mean(
            m.educational_value for m in metrics_list
        )
    ...
    def _generate_optimization_recommendations(
        ...
        if len(ranked_providers) > 1:
            accuracy_leader = max(
                ranked_providers, key=lambda x: x.average_metrics.accuracy_score
            )
            speed_leader = min(
                ranked_providers, key=lambda x: x.average_metrics.response_time
            )
        ...
        educational_scores = [
            p.average_metrics.educational_value for p in ranked_providers
        ]
        ...
        high_variance_providers = [
            p for p in ranked_providers if p.score_variance > 0.15
        ]
    ...
    def get_historical_comparisons(
        ...
        return [
            result for result in self.test_results if result.timestamp >= cutoff_date
        ]
