...
class RAGPipelineService:
    ...
    async def execute_pipeline(
        ...
        try:
            ...
            if pipeline_config.enable_caching:
                ...
                if cached_result:
                    logger.debug(
                        f"Returning cached pipeline result for query: {query[:50]}..."
                    )
            ...
            metrics.average_relevance_score = self._calculate_average_relevance(
                diversified_results
            )
            metrics.citation_coverage = len(response_result.citations) / max(
                len(context_result.citations), 1
            )
        ...
        try:
            ...
            for i, result in enumerate(results):
                if isinstance(result, Exception):
                    logger.error(
                        f"Batch pipeline failed for query '{queries[i]}': {result}"
                    )
                    # Create error result
                    error_result = self._create_empty_result(
                        ...
                        PipelineMetrics(),
                        PipelineStage.QUERY_PROCESSING,
                        errors=[str(result)],
                    )
    ...
    async def _execute_diversification_stage(
        ...
        try:
            ...
            logger.debug(
                f"Diversified {len(results)} -> {len(diversified_results)} results"
            )
    ...
    async def _execute_context_composition_stage(
        ...
        try:
            ...
            context_result = self.context_composition.compose_context(
                search_results=results, query=query, config=composition_config
            )
    ...
    async def _execute_response_generation_stage(
        self, query: str, context_result: ContextResult, config: RAGPipelineConfig
    ) -> ResponseResult:
        ...
        try:
            ...
            response_result = await self.response_generation.generate_response(
                context_result=context_result, query=query, config=generation_config
            )
...
def create_rag_pipeline_service(
    ...
    **service_kwargs,
    ...
    if "dense_retrieval_service" not in service_kwargs:
        service_kwargs["dense_retrieval_service"] = DenseRetrievalService(
            settings=settings
        )

    if "sparse_retrieval_service" not in service_kwargs:
        service_kwargs["sparse_retrieval_service"] = SparseRetrievalService(
            settings=settings
        )
    ...
    return RAGPipelineService(config=config, settings=settings, **service_kwargs)
