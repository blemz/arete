...
class ContextCompositionService:
    ...
    def compose_context(
        ...
        try:
            ...
            if composition_config.enable_caching:
                ...
                if cached_result:
                    logger.debug(
                        f"Returning cached composition for query: {query[:50]}..."
                    )
            ...
            try:
                ...
                results.append(
                    self._create_empty_result(
                        query, config.strategy if config else self.config.strategy
                    )
                )
    ...
    def _intelligent_stitching_composition(
        ...
        logger.debug(
            f"Applying intelligent stitching for {len(search_results)} results"
        )
        ...
        for _doc_id, doc_results in document_groups.items():
            ...
            non_overlapping_results = self._remove_overlaps(
                doc_results, config.overlap_threshold
            )
            overlaps_detected += len(doc_results) - len(non_overlapping_results)

            # Create coherent passage groups
            coherent_groups = self._create_coherent_groups(
                non_overlapping_results, config.coherence_threshold
            )
        ...
        # Process citations
        citation_contexts = (
            self._process_citations(citations, composed_text, config)
            if citations
            else []
        )

        # Check if content was truncated due to token limits
        was_truncated = total_tokens >= config.max_tokens or any(
            self.count_tokens(result.chunk.text) > config.max_tokens
            for result in search_results
            if passage_groups == []
        )
    ...
    def _semantic_grouping_composition(
        ...
        semantic_groups = self._group_by_semantic_similarity(
            search_results, config.coherence_threshold
        )
        ...
        for group_results in semantic_groups:
            ...
            avg_score = sum(r.relevance_score for r in group_results) / len(
                group_results
            )
        ...
        for result in sorted_results:
            ...
            if total_tokens + chunk_tokens > config.max_tokens:
                ...
                if remaining_tokens > 50:  # Minimum meaningful content
                    ...
                    group = PassageGroup(
                        chunks=[truncated_chunk], coherence_score=result.relevance_score
                    )
                    passage_groups.append(group)
                    total_tokens += remaining_tokens
                break

            group = PassageGroup(chunks=[chunk], coherence_score=result.relevance_score)
            passage_groups.append(group)
        ...
        for result in results[1:]:
            ...
            for existing in non_overlapping:
                similarity = self._calculate_text_similarity(
                    result.chunk.text, existing.chunk.text
                )
    ...
    def _group_by_semantic_similarity(
        ...
        while remaining_results:
            ...
            for i, result in enumerate(remaining_results):
                similarity = self._calculate_text_similarity(
                    seed.chunk.text, result.chunk.text
                )
                if similarity >= threshold:
                    group.append(result)
                    to_remove.append(i)
    ...
    def _calculate_performance_metrics(self, result: ContextResult) -> dict[str, Any]:
        ...
        return {
            ...
            "groups_per_token": len(result.passage_groups)
            / max(result.total_tokens, 1),
            "citations_per_group": len(result.citations)
            / max(len(result.passage_groups), 1),
            "composition_speed": result.total_tokens
            / max(result.composition_time, 0.001),  # tokens per second
        }

    def _create_empty_result(
        self, query: str, strategy: CompositionStrategy
    ) -> ContextResult:
