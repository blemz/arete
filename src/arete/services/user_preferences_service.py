...
class UserPreferencesService:
    ...
    def update_user_preferences(
        ...
        self._record_preference_change(
            user_id, "updated", f"Updated preferences: {', '.join(changes)}"
        )
    ...
    def migrate_user_preferences(
        ...
        self._preferences[user_id] = preferences
        self._record_preference_change(
            user_id, "migrated", "Preferences migrated to new format"
        )
    ...
    def reset_user_preferences_to_defaults(self, user_id: str) -> bool:
        ...
        try:
            ...
            self._record_preference_change(
                user_id, "reset", "Preferences reset to defaults"
            )
            return True
        except Exception:
            return False
    ...
    def _detect_preference_changes(
        ...
        if (
            old_prefs.notification_settings.to_dict()
            != new_prefs.notification_settings.to_dict()
        ):
    ...
    def _record_preference_change(
        self, user_id: str, action: str, description: str
    ) -> None:
        ...
        if len(self._preferences_history[user_id]) > 50:
            self._preferences_history[user_id] = self._preferences_history[user_id][
                -50:
            ]
