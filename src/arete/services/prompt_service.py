...
class PromptTemplateFactory:
    ...
    def get_template(
        self, provider: str, prompt_type: PromptType
    ) -> BasePromptTemplate:
        """
...
class PromptService:
    ...
    async def generate_tutoring_response(
        ...
        # Generate prompt
        prompt_result = await self.generate_prompt(
            context, request.prompt_type, provider
        )
        ...
        logger.info(
            f"Generated tutoring response: {len(response.content)} chars, "
    ...
    async def generate_prompt(
        ...
        logger.info(
            f"Generated {prompt_type.value} prompt for {provider}: "
    ...
    def get_provider_info(self) -> dict[str, Any]:
        ...
        return {
            ...
            "supported_prompt_types": [
                pt.value for pt in self.get_supported_prompt_types()
            ],
