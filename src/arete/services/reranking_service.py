...
class RerankingResult:
    ...
    def get_final_score(
        ...
        if combination_method == "rerank_only":
            ...
            return (
                self.original_result.relevance_score * orig_w
...
class RerankingService:
    ...
    def _initialize_models(self):
        ...
        try:
            ...
            if self.config.method in [
                RerankingMethod.SEMANTIC_SIMILARITY,
                RerankingMethod.HYBRID,
            ]:
            ...
            logger.info(
                f"Initialized RerankingService with method: {self.config.method}"
            )
    ...
    def _initialize_cross_encoder(self):
        ...
        try:
            ...
            logger.warning(
                "sentence-transformers not available, using mock cross-encoder"
            )
    ...
    def rerank(
        ...
        try:
            ...
            if self.config.enable_caching and cache_key in self._result_cache:
                ...
                if (
                    ...
                    self.metrics.record_query(
                        method, len(search_results), 0.0, cache_hit=True
                    )
            ...
            final_results = sorted(
                filtered_results, key=lambda x: x.rerank_score, reverse=True
            )[:top_k]
            ...
            logger.info(
                f"Re-ranked {len(search_results)} results to {len(final_results)} "
    ...
    def _rerank_hybrid(
        ...
        for ce_result, sem_result in zip(cross_encoder_results, semantic_results, strict=False):
            ...
            hybrid_result = RerankingResult(
                ...
                new_rank=0,
                score_improvement=hybrid_score
                - ce_result.original_result.relevance_score,
    ...
    def _get_cache_key(
        ...
        result_ids = [
            str(result.chunk.id) for result in search_results[:10]
...
def create_reranking_service(
    ...
    """
    return RerankingService(config=config, settings=settings)
