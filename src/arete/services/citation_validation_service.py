...
class CitationValidationService:
    ...
    def __init__(self, config: CitationValidationConfig | None = None):
        ...
        logger.info(
            f"Initialized CitationValidationService with {len(self.validation_rules)} rules"
        )
    ...
    async def validate_citations_batch(
        ...
        try:
            ...
            for citation in citations:
                doc = (
                    doc_mapping.get(citation.document_id)
                    if original_documents
                    else None
                )
    ...
    async def _apply_validation_rule(
        ...
        if rule.validation_type == ValidationType.TEXTUAL_ACCURACY:
            ...
            return await self._validate_source_attribution(
                citation, source_context, rule
            )

        elif rule.validation_type == ValidationType.CONTEXTUAL_RELEVANCE:
            return await self._validate_contextual_relevance(
                citation, source_context, rule
            )
    ...
    async def _validate_textual_accuracy(
        ...
        if not source_context or not source_context.citations:
            return {
                "passed": True,
                "score": 0.5,
                "message": "No source context available",
            }
        ...
        for ctx in source_context.citations:
            similarity = difflib.SequenceMatcher(
                None, citation.text.lower(), ctx.citation.text.lower()
            ).ratio()
    ...
    async def _validate_source_attribution(
        ...
        if source_context and source_context.citations:
            for ctx in source_context.citations:
                ...
                if (
                    citation.source_author.lower() in source_cit.source_author.lower()
                    and citation.source_title.lower() in source_cit.source_title.lower()
                ):
                    return {
                        "passed": True,
                        "score": 1.0,
    ...
    async def _verify_source_accuracy(
        ...
        for ctx in source_context.citations:
            ...
            attr_match = 0.0
            if (
                citation.source_author.lower() in ctx.citation.source_author.lower()
                and citation.source_title.lower() in ctx.citation.source_title.lower()
            ):
                attr_match = 1.0
    ...
    def _generate_improvement_suggestions(
        ...
        if result.confidence_score < 0.7:
            suggestions.append(
                "Consider verifying the citation against the original source"
            )
        ...
        if result.attribution_accuracy < 0.8:
            ...
            if not citation.source_reference:
                suggestions.append(
                    "Add specific page or section reference (e.g., 'Republic 514a')"
                )

        # Contextual relevance suggestions
        if result.contextual_relevance < 0.6:
            suggestions.append(
                "Consider the relevance of this citation to your argument"
            )
            suggestions.append(
                "Provide more context to explain the citation's significance"
            )
    ...
    def _calculate_batch_metrics(
        self, batch_result: BatchValidationResult
    ) -> BatchValidationResult:
        ...
        total_confidence = sum(
            r.confidence_score for r in batch_result.citation_results
        )
        batch_result.average_confidence = total_confidence / len(
            batch_result.citation_results
        )
        ...
        good_source_accuracy = sum(
            1 for r in batch_result.citation_results if r.source_accuracy >= 0.7
        )
        batch_result.coverage_score = (
            good_source_accuracy / batch_result.total_citations
        )
        ...
        for r in batch_result.citation_results:
            quality = (
                r.confidence_score + r.source_accuracy + r.contextual_relevance
            ) / 3
    ...
    def _assess_batch_quality(
        ...
        quote_count = sum(
            1 for c in citations if c.citation_type == CitationType.DIRECT_QUOTE
        )
