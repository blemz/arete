...
class WeaviateClient:
    ...
    def connect(self) -> None:
        ...
        try:
            ...
            # Now that gRPC port is exposed, we can use it properly
            self.client = weaviate.connect_to_local(
                host=host,
                port=port,
                headers=self.headers,
                grpc_port=50051,  # Explicitly specify gRPC port
                skip_init_checks=False,  # Can enable checks now that gRPC is available
            )
    ...
    def save_document(self, document: Document) -> str:
        ...
        try:
            ...
            result = collection.data.insert(properties=properties, uuid=document.id)
                ...
                results.append(
                    {
                        "document": document,
                        "score": (
                            obj.metadata.score if hasattr(obj, "metadata") else 0.0
                        ),
                    }
                )
            ...
            with self.client.batch as batch:
                for document in documents:
                    properties = {
                        ...
                        "neo4j_id": str(
                            document.id
                        ),  # Use Document ID as neo4j_id reference
                    }

                    batch.add_object(
                        class_name="Document", properties=properties, uuid=document.id
                    )
    ...
    def save_entity(self, entity: Entity) -> str:
        ...
        try:
            ...
            result = collection.data.insert(properties=properties, uuid=entity.id)
    ...
    def create_object(
        ...
        try:
            ...
            raise DatabaseQueryError(
                f"Failed to create {class_name} object: {str(e)}"
            ) from e
    ...
    def create_objects_batch(
        ...
        try:
            ...
            for obj in objects:
                ...
                if vector:
                    batch_data.append({"properties": properties, "vector": vector})
                else:
                    batch_data.append({"properties": properties})
            ...
            raise DatabaseQueryError(
                f"Failed to batch create {class_name} objects: {str(e)}"
            ) from e
        ...
        try:
            ...
            for obj in response:
                result = {
                    ...
                    "_additional": {
                        "certainty": (
                            obj.metadata.certainty if obj.metadata else min_certainty
                        )
                    },
                    "metadata": {
                        "certainty": (
            ...
            raise DatabaseQueryError(
                f"Vector search failed for {collection_name}: {str(e)}"
            ) from e
