...
class EntityRepository(GraphRepository[Entity], SearchableRepository[Entity]):
    ...
    async def get_by_id(self, entity_id: UUID | str) -> Entity | None:
        ...
        """
        try:
            result = await self._neo4j_client.get_node_by_id(str(entity_id), "Entity")
    ...
    async def delete(self, entity_id: UUID | str) -> bool:
        ...
        try:
            ...
            await self._weaviate_client.delete_entity("Entity", str(entity_id))
    ...
    async def search_entities(
        ...
        try:
            ...
            weaviate_results = await self._weaviate_client.search_near_text(
                collection="Entity", query=query, limit=limit, certainty=0.6
            )
    ...
    async def create_relationship(
        ...
        try:
            print(
                f"DEBUG: Creating relationship: {source_entity_id} -{relationship_type}-> {target_entity_id}"
            )
            logger.info(
                f"Creating relationship: {source_entity_id} -{relationship_type}-> {target_entity_id}"
            )
            ...
            print(
                f"DEBUG: Entity existence check: source={source_exists}, target={target_exists}"
            )
            logger.info(
                f"Entity existence check: source={source_exists}, target={target_exists}"
            )

            if not source_exists or not target_exists:
                print(
                    f"DEBUG: One or both entities not found: {source_entity_id}, {target_entity_id}"
                )
                raise EntityNotFoundError(
                    f"One or both entities not found: {source_entity_id}, {target_entity_id}"
                )
            ...
            sanitized_rel_type = (
                relationship_type.replace(" ", "_").replace("-", "_").upper()
            )

            logger.info(
                f"Sanitized relationship type: {relationship_type} -> {sanitized_rel_type}"
            )
        ...
        try:
            ...
            for triple in triples:
                ...
                if not source_id or not target_id:
                    logger.warning(
                        f"Skipping triple - entities not found: {subject_name} -> {object_name}"
                    )
                    continue
                ...
                try:
                    ...
                    logger.warning(
                        f"Failed to create relationship for triple {subject_name} -{relation_type}-> {object_name}: {str(e)}"
                    )
                    continue

            logger.info(
                f"Successfully created {created_count} relationships from {len(triples)} triples"
            )
