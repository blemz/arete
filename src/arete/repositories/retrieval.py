...
class RetrievalRepository:
    ...
    def __init__(
        ...
        if sparse_service is None:
            ...
            self.sparse_service = create_sparse_retrieval_service(
                retriever_type="bm25", settings=self.settings
            )
        else:
        ...
        if graph_service is None:
            ...
            self.graph_service = GraphTraversalService(
                neo4j_client=neo4j_client, settings=self.settings
            )
        else:
        ...
        logger.info(
            "Initialized RetrievalRepository with dense, sparse, and graph services"
        )
    ...
    def _hybrid_search(
        ...
        **kwargs,
        ...
        filtered_results = [
            result for result in combined_results if result.final_score >= min_relevance
        ]
    ...
    def _reciprocal_rank_fusion(
        ...
        for chunk_id in all_chunk_ids:
            ...
            if chunk_id in dense_map and chunk_id in sparse_map:
                base_result = (
                    dense_map[chunk_id]
                    if dense_map[chunk_id].final_score
                    >= sparse_map[chunk_id].final_score
    ...
    def _score_threshold_fusion(
        ...
        remaining_dense = [
            r for r in dense_results if str(r.chunk.id) not in seen_chunk_ids
        ]
        remaining_sparse = [
            r for r in sparse_results if str(r.chunk.id) not in seen_chunk_ids
        ]
        ...
        **kwargs,
        ...
        try:
            ...
            graph_query = self.graph_service.generate_cypher_query(
                entities=entities, query_type="entity_lookup"
            )
        ...
        **kwargs,
        ...
        try:
            ...
            final_results = [
                ...
                if getattr(result, "final_score", result.relevance_score)
                >= min_relevance
            ]

            # Sort by enhanced scores
            final_results.sort(
                key=lambda r: getattr(r, "final_score", r.relevance_score), reverse=True
            )
    ...
    def _search_result_to_graph_result(
